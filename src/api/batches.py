...
async def create_batches(
    requests: List[BatchCreateRequest],
    ...
    async with db.begin():
        for req in requests:
            ...
            work_center = await work_center_repo.get_or_create(
                identifier=req.ИдентификаторРЦ,
                name=req.РабочийЦентр
            )
    ...
    for batch in created_batches:
        ...
        for subscription in subscriptions:
            payload = webhook_service.create_webhook_payload("batch_created", {
                "id": batch.id,
...
async def update_batch(
    ...
    for subscription in subscriptions:
        payload = webhook_service.create_webhook_payload("batch_updated", {
            "id": batch.id,
            "batch_number": batch.batch_number,
    ...
    if data.is_closed and batch.is_closed:
        ...
        for subscription in subscriptions:
            ...
            payload = webhook_service.create_webhook_payload("batch_closed", {
                "id": batch.id,
                "batch_number": batch.batch_number,
